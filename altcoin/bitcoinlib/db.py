...
class DbInit:
    ...
    def __init__(self, db_uri=None):
        if db_uri is None:
            _logger.warning("Error when configuring database " )
...
class DbWallet(Base):
    ...
    username = Column(String(50), doc="Wallet owner")
...
class DbKey(Base):
    ...
    def __repr__(self):
        return "<DbKey(id='%s', name='%s', address='%s', wif='%s', private='%s', public='%s'>" % (self.id, self.name, self.address,  self.wif,self.private, self.public)

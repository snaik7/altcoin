...
def wallets_list_user(db_uri=None, username=None, include_cosigners=False, network=None):
    """
    List Wallets from database
    ...
    session = DbInit(db_uri=db_uri).session
    wallets = session.query(DbWallet).filter_by(username=username).order_by(DbWallet.id).all()
    wlst = []
    for w in wallets:
        if w.parent_id and not include_cosigners:
            continue

        service_url = get_env_var('service.url') + '/unspent/?wallet_id=' + str(w.id) + '&network=' + str(network)+'&withsum=true'
        print('service_url for  unspent', service_url)
        resp = requests.get(service_url, auth=utils.service_auth)
        ...
        if resp.status_code != 200:
            ...
            if  wallet_address_list:
                count = len(wallet_address_list)
                balance = wallet_address_list[count-1].get('toal_unspent')
            else:
                balance = 0
...
def get_wallet(db_uri=None, wallet_id=None, include_cosigners=False):
    ...
    session = DbInit(db_uri=db_uri).session
    wallets = session.query(DbWallet).filter_by(id=wallet_id).order_by(DbWallet.id).all()
    wlst = []
    for w in wallets:
        if w.parent_id and not include_cosigners:
            continue
...
def rename_wallet(db_uri=None, wallet_id=None, name=None) :

    session = DbInit(db_uri=db_uri).session
    wallets = session.query(DbWallet).filter_by(id=wallet_id)
    wlst = []
    if not wallets or not wallets.first():
        session.close()
        raise WalletError("Wallet '%s' not found" % wallet_id)
    else:
        session.query(DbWallet).filter_by(id=wallet_id).update({DbWallet.name: name})
        session.commit()
        for w in wallets:
            wlst.append({
                'id': w.id,
...
def wallet_create_or_open(
        ...
        key_path=None, username=None, db_uri=None):
    """
...
#        return HDWallet(name, db_uri=db_uri)
        ...
        return HDWallet.create(name, keys, owner, network, account_id, purpose, scheme, sort_keys,
                               ...
                               key_path, username, db_uri=db_uri)
...
def wallet_create_or_open_multisig(
        ...
        witness_type=DEFAULT_WITNESS_TYPE, encoding=None, cosigner_id=None, key_path=None, username=None,
...
class HDWallet(object):
    ...
    def _create(cls, name, key, owner, network, account_id, purpose, scheme, parent_id, sort_keys,
                witness_type, encoding, multisig, sigs_required, cosigner_id, key_path, username, db_uri):
        ...
        new_wallet = DbWallet(name=name, owner=owner, network_name=network, purpose=purpose, scheme=scheme,
                              ...
                              key_path=key_path, username=username)
    ...
    def create(cls, name, keys=None, owner='', network=None, account_id=0, purpose=0, scheme='bip32',
               ...
               cosigner_id=None, key_path=None, username=None, db_uri=None):
        """
        ...
        hdpm = cls._create(name, key, owner=owner, network=network, account_id=account_id, purpose=purpose,
                           ...
                           key_path=main_key_path, username=username, db_uri=db_uri)
        ...
        if multisig:
            ...
            for cokey in hdkey_list:
                ...
                w = cls._create(name=wn, key=cokey, owner=owner, network=network, account_id=account_id,
                                ...
                                sigs_required=None, cosigner_id=wlt_cos_id, key_path=c_key_path,username=username,
                                db_uri=db_uri)
    ...
    def create_multisig(cls, name, keys, sigs_required=None, owner='', network=None, account_id=0, purpose=None,
                        ...
                        cosigner_id=None, username=None, db_uri=None):
        """
        ...
        return cls.create(name, keys=keys, owner=owner, network=network, account_id=account_id, purpose=purpose,
                          ...
                          sigs_required=sigs_required, cosigner_id=cosigner_id, key_path=key_path, username=username,
                          db_uri=db_uri)
